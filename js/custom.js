...
$(document).ready(function () {
	...
	$('#view').on('click', '.btn-form-ubah', function () { // Ketika tombol dengan class btn-form-ubah pada div view di klik
		id_siswa = $(this).data('id_siswa') // Set variabel id dengan id yang kita set pada atribut data-id pada tag button edit
		...
		var nisn = tr.find('.nisn-value').val() // Ambil id_user dari input type hidden
		...
		$('#nisn').val(nisn) // Set value dari textbox id_user yang ada di form
	...
	$('#view').on('click', '.btn-alert-hapus', function () { // Ketika tombol dengan class btn-alert-hapus pada div view di klik
		id_siswa = $(this).data('id_siswa') // Set variabel id dengan id yang kita set pada atribut data-id pada tag button hapus
	})
	...
	$('#btn-simpan').click(function () { // Ketika tombol simpan di klik
		...
		$.ajax({
			url: base_url + 'Data_siswa/simpan', // URL tujuan
	...
	$('#btn-ubah').click(function () { // Ketika tombol ubah di klik
		...
		$.ajax({
			url: base_url + 'Data_siswa/ubah/' + id_siswa, // URL tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: $("#form-modal form").serialize(), // Ambil semua data yang ada didalam tag form
			dataType: 'json',
			beforeSend: function (e) {
	...
	$('#btn-hapus').click(function () { // Ketika tombol hapus di klik
		...
		$.ajax({
			url: base_url + 'Data_siswa/hapus/' + id_siswa, // URL tujuan
